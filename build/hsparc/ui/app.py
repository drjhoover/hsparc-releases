...
from hsparc.ui.widgets.study_instructions_dialog import StudyInstructionsDialog
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Store unlocked study PINs in memory for automatic encryption
        self._unlocked_studies = {}  # {study_id: pin}
        ...
        title = QLabel("HSPARC – Studies")
        title.setStyleSheet("font-size:18px;font-weight:600;")
        header_row.addWidget(title)

        header_row.addStretch(1)

        # Admin and Shutdown buttons (only in kiosk mode)
        if self.kiosk_mode:
            # Admin button
            self.btn_admin = QPushButton("⚙ Admin", self)
            ...
            """)
            self.btn_admin.setToolTip("Admin: Kill session and return to login")
            self.btn_admin.clicked.connect(self._admin_unlock)
        ...
        self.btn_import_study = QPushButton("Import Study…")
        self.btn_export_study = QPushButton("Export Study…")
        self.btn_delete_study = QPushButton("Delete Study…")
    ...
    def _admin_unlock(self):
        """Exit to Ubuntu login screen - kills hsparc user session."""
        reply = QMessageBox.question(
            self,
            "Exit to Login Screen",
            "Exit HSPARC and return to Ubuntu login screen?\n\n"
            "This will close all windows and end the current session.\n"
            "You can then select which user to log in as.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No
        )
        ...
        try:
            print("[main] Exiting to login screen - killing hsparc user session...")
            ...
            # Kill the hsparc user session (no sudo needed - user can kill own session)
            subprocess.Popen(["pkill", "-u", "hsparc"])

            # Force quit application
            QApplication.quit()

        except Exception as e:
            print(f"[main] Error killing session: {e}")
            # Even if pkill fails, still quit the app
            QApplication.quit()
    ...
    def refresh_studies(self):
        ...
        with dbm.get_session() as s:
            ...
            for study in studies:
                lock_icon = "🔒 "
                item = QListWidgetItem(f"{lock_icon}{study.label}  —  {study.id[:8]}")
    ...
    def create_study(self):
        ...
        pin = PinDialog.get_pin_from_user(
            ...
            "⚠️ Store this PIN securely - it cannot be recovered!"
        )
        ...
        with dbm.get_session() as s:
            ...
            while attempts < max_attempts:
                ...
                if verify_pin(study.security_hash, pin):
                    ...
                    # Store PIN in memory for automatic video encryption
                    self._unlocked_studies[study_id] = pin
                    break
                else:
                    attempts += 1
            ...
            # Pass study PIN to StudyWindow
            w = StudyWindow(study.id, study.label, study_pin=pin)
    ...
    def export_study(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                f"File: {export_path}\n\n"
                f"⚠️ Keep the PIN secure - it's required to import this study!"
            )
    ...
    def delete_study(self):
        ...
        reply = QMessageBox.warning(
            self,
            "Delete Study",
            f"⚠️ DELETE STUDY: '{study_label}'\n\n"
            f"This will permanently delete:\n"
            f"  • {rec_count} recording(s) and video file(s)\n"
            f"  • All observer sessions and input data\n"
...
class StudyWindow(QMainWindow):
    def __init__(self, study_id: str, study_label: str, study_pin: Optional[str] = None):
        super().__init__()
        self.study_id = study_id
        self.study_pin = study_pin  # Store PIN for passing to recorder and observer
        ...
        self.btn_back = QPushButton("← Back to Studies")
        ...
        # First row: Recording management
        row1 = QHBoxLayout()
        self.btn_new_rec = QPushButton("Record Video…")
        ...
        row1.addWidget(self.btn_new_rec)
        row1.addWidget(self.btn_import)
        row1.addWidget(self.btn_observer)
        row1.addWidget(self.btn_researcher)
        left.addLayout(row1)

        # Second row: Study management and instructions
        row2 = QHBoxLayout()
        self.btn_instructions = QPushButton("📋 Edit Observer Instructions…")
        self.btn_instructions.setToolTip(
            "Set up instructions that will be shown to participants\n"
            "before each observer session in this study."
        )
        ...
        """)
        row2.addWidget(self.btn_instructions)

        row2.addStretch(1)  # Push delete button to the right

        self.btn_delete_rec = QPushButton("Delete Recording…")
        self.btn_delete_rec.setStyleSheet("background-color: #d9534f; color: white;")
        row2.addWidget(self.btn_delete_rec)
        left.addLayout(row2)
        ...
        self.btn_instructions.clicked.connect(self._edit_instructions)
    ...
    def refresh_recordings(self):
        self.rec_list.clear()
        with dbm.get_session() as s:
            recordings = (s.query(dbm.Recording)
                          .filter_by(case_id=self.study_id)
                          .order_by(dbm.Recording.created_utc.desc()).all())

            print(f"[app] Found {len(recordings)} recordings for study {self.study_id[:8]}")

            for r in recordings:
                print(f"[app] Recording {r.id[:8]}: video_path={r.video_path}")
                if r.video_path:
                    exists = Path(r.video_path).exists() if r.video_path else False
                    print(f"[app]   File exists: {exists}")

                txt = f"{r.created_utc.strftime('%Y-%m-%d %H:%M')} — {r.id[:8]}"
                if r.video_path:
                    txt += " (video)"
    ...
    def open_recorder(self):
        w = RecorderWindow(parent=self, case_id=self.study_id, study_pin=self.study_pin)
    ...
    def load_recording(self, curr, prev):
        ...
        if path.endswith('.enc'):
            ...
            self.video_title.setText(f"🔒 {Path(path).stem} (encrypted)")
            self._show_cover(True)
            self.lbl_status.setText("Encrypted video - use Observer or Researcher mode to view")
            return

        # Load plain video for preview
        self.video_title.setText(Path(path).name)
        self.player.setSource(QUrl.fromLocalFile(path))
        self.lbl_status.setText("Ready to play")
    ...
    def open_observer_mode(self):
        ...
        try:
            ...
            if not rec_id:
                ...
                return
            if not video_path:
                QMessageBox.warning(self, "Missing video", "Recording has no video file.")
                return

            # Check if file exists (allow both .mp4 and .enc files)
            ...
            print(f"[app] Opening observer with video: {video_path}")
            print(f"[app] File exists: {video_path_obj.exists()}")
            print(f"[app] Is encrypted: {video_path_obj.suffix == '.enc'}")
            print(f"[app] Study PIN available: {self.study_pin is not None}")
            ...
            w = ObserverWindow(
                ...
                study_pin=self.study_pin  # Pass PIN for decryption
            )
            self._child_windows.append(w)
            w.show()
            w.activateWindow()
    ...
    def open_researcher_mode(self):
        ...
        if not rec_id:
            return
        w = ResearcherWindow(recording_id=rec_id, study_pin=self.study_pin)
        self._child_windows.append(w)
        w.show()

    def _edit_instructions(self):
        """Open dialog to edit observer instructions for this study."""
        try:
            StudyInstructionsDialog.edit_instructions(self, study_id=self.study_id)

            # Show a preview of current instructions
            with dbm.get_session() as s:
                study = s.query(dbm.Study).filter_by(id=self.study_id).first()
                if study and study.observer_instructions_text:
                    preview = study.observer_instructions_text[:100]
                    if len(study.observer_instructions_text) > 100:
                        preview += "..."
                    has_image = "with image" if study.observer_instructions_image else "no image"
                    self.statusBar().showMessage(
                        f"Instructions set ({has_image}): {preview}",
                        5000
                    )
                else:
                    self.statusBar().showMessage("No instructions set for this study", 3000)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not edit instructions:\n{e}")
    ...
    def delete_recording(self):
        ...
        reply = QMessageBox.warning(
            self,
            "Delete Recording",
            f"⚠️ DELETE RECORDING from {rec_date}\n\n"
            f"This will permanently delete:\n"
            f"  • The video file\n"
            f"  • {session_count} observer session(s)\n"
            f"  • All associated input streams and events\n"
            f"  • All metadata\n\n"
            f"THIS CANNOT BE UNDONE!\n\n"
