...
class ObserverInstructionsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Helper text
        helper = QLabel("Press the South button (A/X) on your controller to continue")
        ...
        for device in self._devices:
            ...
            try:
                for event in device.read():
                    # Debounce
                    if current_time - self._last_input_time.get(device_path, 0) < 0.3:
                        continue

                    # South button (A/X) confirms ready
