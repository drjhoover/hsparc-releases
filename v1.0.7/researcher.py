...
class ResearcherWindow(QMainWindow):
    ...
    def _open_av_settings(self):
        ...
        dialog = AVSettingsDialog(self)
        dialog.exec()
    def _download_analysis_report(self):
        """Download analysis results as Word document."""
        ...
        try:
            ...
            default_name = f"analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
            ...
            print(f"[researcher] Generating analysis report...")
            
            # Create document
            doc = Document()
            
            # Title
            title = doc.add_heading('HSPARC Analysis Report', 0)
            title.alignment = WD_ALIGN_PARAGRAPH.CENTER
            
            # Metadata
            doc.add_paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            doc.add_paragraph(f"Case ID: {self._case_id or 'N/A'}")
            doc.add_paragraph(f"Number of Traces: {len(self._last_analysis_trace_data) if self._last_analysis_trace_data else 0}")
            doc.add_paragraph()
            
            results = self._last_analysis_results
            
            # Add summary text if available
            if hasattr(results, 'summary_text') and results.summary_text:
                doc.add_heading('Summary', 1)
                doc.add_paragraph(results.summary_text)
                doc.add_paragraph()
            
            # Handle different analysis types
            analysis_type = getattr(results, 'analysis_type', 'unknown')
            
            # Single trace analysis
            if analysis_type == 'single' and results.single:
                single = results.single
                
                doc.add_heading('Single Trace Analysis', 1)
                doc.add_paragraph(f"Trace: {single.trace_name}")
                
                # Descriptive stats
                if hasattr(single, 'mean'):
                    doc.add_heading('Descriptive Statistics', 2)
                    table = doc.add_table(rows=6, cols=2)
                    table.style = 'Light Grid Accent 1'
                    
                    stats = [
                        ('Mean', f"{single.mean:.4f}"),
                        ('Std Dev', f"{single.std:.4f}"),
                        ('Min', f"{single.min:.4f}"),
                        ('Max', f"{single.max:.4f}"),
                        ('Median', f"{single.median:.4f}"),
                        ('Range', f"{single.max - single.min:.4f}"),
                    ]
                ...
                # Change points
                if hasattr(single, 'change_points') and single.change_points:
                    doc.add_heading('Significant Changes', 2)
                    doc.add_paragraph(f"Detected {len(single.change_points)} significant changes:")
                    
                    table = doc.add_table(rows=min(len(single.change_points), 20) + 1, cols=2)
                    table.style = 'Light Grid Accent 1'
                    table.rows[0].cells[0].text = 'Time (s)'
                    table.rows[0].cells[1].text = 'Magnitude'
                    
                    for i, cp in enumerate(single.change_points[:20]):
                        table.rows[i+1].cells[0].text = f"{cp['time_s']:.2f}"
                        table.rows[i+1].cells[1].text = f"{cp['magnitude']:.4f}"
            
            # Pairwise analysis
            elif analysis_type == 'pairwise' and results.pairwise:
                pair = results.pairwise
                
                doc.add_heading('Pairwise Analysis', 1)
                doc.add_paragraph(f"Trace 1: {pair.trace1_name}")
                doc.add_paragraph(f"Trace 2: {pair.trace2_name}")
                doc.add_paragraph()
                
                # Correlation
                if hasattr(pair, 'correlation'):
                    doc.add_heading('Correlation', 2)
                    doc.add_paragraph(f"Pearson Correlation: {pair.correlation:.4f}")
                    if hasattr(pair, 'correlation_p'):
                        doc.add_paragraph(f"P-value: {pair.correlation_p:.6f}")
                        sig = "Yes" if pair.correlation_p < 0.05 else "No"
                        doc.add_paragraph(f"Statistically Significant (p < 0.05): {sig}")
                    doc.add_paragraph()
                
                # Convergence/Divergence
                if hasattr(pair, 'convergence_pct'):
                    doc.add_heading('Convergence/Divergence Analysis', 2)
                    doc.add_paragraph(f"Overall Pattern: {pair.pattern if hasattr(pair, 'pattern') else 'N/A'}")
                    doc.add_paragraph(f"Convergence: {pair.convergence_pct:.1f}%")
                    doc.add_paragraph(f"Divergence: {pair.divergence_pct:.1f}%")
                    doc.add_paragraph()
                    
                    # Critical moments
                    if hasattr(pair, 'critical_moments') and pair.critical_moments:
                        doc.add_heading('Critical Moments', 3)
                        table = doc.add_table(rows=min(len(pair.critical_moments), 20) + 1, cols=3)
                        table.style = 'Light Grid Accent 1'
                        table.rows[0].cells[0].text = 'Time (s)'
                        table.rows[0].cells[1].text = 'Type'
                        table.rows[0].cells[2].text = 'Distance'
                        
                        for i, moment in enumerate(pair.critical_moments[:20]):
                            table.rows[i+1].cells[0].text = f"{moment['time_s']:.2f}"
                            table.rows[i+1].cells[1].text = moment['type'].title()
                            table.rows[i+1].cells[2].text = f"{moment['distance']:.4f}"
            
            # Multi-trace analysis
            elif analysis_type == 'multi' and results.multi:
                multi = results.multi
                
                doc.add_heading('Multi-Trace Analysis', 1)
                doc.add_paragraph(f"Number of traces: {len(multi.trace_names)}")
                doc.add_paragraph(f"Traces: {', '.join(multi.trace_names)}")
                doc.add_paragraph()
                
                # Correlation matrix
                if hasattr(multi, 'correlation_matrix') and multi.correlation_matrix is not None:
                    doc.add_heading('Correlation Matrix', 2)
                    doc.add_paragraph("See visualization in analysis dialog for correlation heatmap.")
                    doc.add_paragraph()
                
                # PCA
                if hasattr(multi, 'pca_variance') and multi.pca_variance:
                    doc.add_heading('Principal Component Analysis', 2)
                    doc.add_paragraph("Explained Variance by Component:")
                    for i, var in enumerate(multi.pca_variance[:5]):
                        doc.add_paragraph(f"  PC{i+1}: {var:.1f}%")
                    doc.add_paragraph()
                
                # Clustering
                if hasattr(multi, 'n_clusters') and multi.n_clusters:
                    doc.add_heading('Clustering Analysis', 2)
                    doc.add_paragraph(f"Optimal number of clusters: {multi.n_clusters}")
                    if hasattr(multi, 'cluster_labels'):
                        doc.add_paragraph(f"Cluster assignments: {dict(zip(multi.trace_names, multi.cluster_labels))}")
            
            # Save document
            doc.save(filepath)
            print(f"[researcher] Report saved to: {filepath}")
            ...
            QMessageBox.critical(
                self,
                "Missing Dependency",
                "The python-docx library is required to generate Word documents.\n\n"
                "Install it with: pip install python-docx"
            )
        except Exception as e:
            print(f"[researcher] Report generation error: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self,
